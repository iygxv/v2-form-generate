<template>
  <div>
    ...
    <el-button type="primary" @click="createDialog.dialogVisible = true">添加字段</el-button>
    <el-dialog
          title="生成Dialog代码"
          width="700px"
          append-to-body
...
<script>
import { createCodegenContext, vueTemplate, vueScript, humpChange } from './utils'
...
export default {
  ...
  methods: {
    ...
    generateCode() {
      return this.generateTemplateCode() + '\n'.repeat(2) + this.generateScriptCode()
    },
    generateTemplateCode() {
      ...
      return vueTemplate(indentLevel => {
        ...
        newLine()
        push(`<el-button type="primary" @click="handle${humpChange(dialogVar)}('multiple')">确定</el-button>`)
        deindent()
        push(`</template>`)
        deindent()
        push(`</el-dialog>`)
    ...
    generateRules(items, indentLevel) {
      ...
      for (let i = 0; i < requireItem.length; i++) {
        push(`${requireItem[i].propsName}: [ { required: true, message: '${requireItem[i].name}不能为空', trigger: ['blur', 'change'] } ]${i !== requireItem.length - 1 ? ',' : ''}`)
